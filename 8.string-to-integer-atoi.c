...
int myAtoi(char* s) {
    ...
    /* Next character could indicate signedness */
    if ((s[i] == '-') || (s[i] == '+')) {
        if (s[i] == '-') { negate = true; }
        ++i;
    }
    ...
    /* Quick exit if the integer is going to be too long (INT_MIN and INT_MAX are 10 digits) */
    if (count > 10) {
        if (negate) { return INT32_MIN; }
        return INT32_MAX;
    }

    /* Convert digits to an integer, each digit (minus the ASCII value) is multiplied by 10^exp where exp is the column value for that digit
       273
       ^   = 2 * 10^2
        ^  = 7 * 10^1
         ^ = 3 * 10^0
       Therefore, result = 200 + 70 + 3 = 273
    */
    if (count > 0) {
        for (int exp = count - 1; pos < i; ++pos, --exp) {
            res += (s[pos] - '0') * pow(10, exp);
        }
    }
